...
#V3_4: {
	...
	#propertyState: "Default" | "Authored" | "Generated" | "Suggested" | *"Default"

	#visibility: "Visible" | "Hidden" | "Children" | *"Visible"

	#visibilityProperty: #visibility | #fullVisibilityProperty

	#fullVisibilityProperty: {
		Value: #visibility
	...
	#sourceType: "Default" | "User" | "Internal" | "External" | *"Default"
	...
	#lsdlReference: {
		...
		// Indicates whether the entity is hidden. Hidden entities are
		// ignored for all purposes other than being available nodes
		// along paths between other related entities.
		...
		// Indicates whether this entity represents the Name or ID of
		// something
		NameType?: "None" | "Name" | "Identifier" | *"None"
		...
		// A list of related entities which are used to group the query
		// when this entity is shown
		ImplicitGroupings?: #entityReferences
	}

	#entityDefinition: #bindingEntityDefinition | #textEntityDefinition

	#bindingEntityDefinition: {
		// The conceptual model object binding
		Binding: #entityBinding
	}

	#entityBinding: #conceptualEntityBinding | #conceptualPropertyBinding | #hierarchyBinding | #hierarchyLevelBinding
	...
	#instances: {
		...
		// suggestions.
		Index?: "Default" | "All" | "None" | *"Default"
	...
	#entityReferences: [...#entityReference] & [_, ...]
	...
	#adjectivePhrasingProperties: #adjectivePhrasingProperty | #antonymPhrasingProperty
	...
	#adverbPhrases: [...#adverbPhrase] & [_, ...]

	#adverbPhrase: #adverbPhraseProperty | #adverbAntonymPhraseProperty

	#adverbPhraseProperty: {
	...
	#example: #nonEmptyString | #fullExample

	#fullExample: struct.MaxFields(1) & {
		{[=~"^.+$" & !~"^()$"]: #exampleProperties}
	}

	#exampleProperties: {
		// Identifies the shared template schema from which the example is
		// derived
		TemplateSchema?: #templateSchema
	}
	...
	#agentProperties: {
		// Indicates the last date and time (in UTC) that this agent
		// modified the linguistic schema.
		LastModified?: #dateTime
	}
}
